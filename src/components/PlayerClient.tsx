  ...
  return <div className="space-y-3">{matches.map((m, idx) => {
    ...
    return <div key={m.id || idx} className="card overflow-hidden">
      <div className="p-4 cursor-pointer hover:bg-[#fafafa] transition-colors" onClick={() => onToggle(isExpanded ? null : (m.id || String(idx)))}>
        <div className="flex items-center justify-between mb-2"><div className="flex items-center gap-3"><span className="text-[#51cf66] font-bold">{wins} 胜利</span><span className="text-[#ddd]">//</span><span className="text-[#ff6b6b] font-bold">{losses} 战败</span></div><span className="text-[10px] text-[#aaa] transform transition-transform" style={{transform: isExpanded ? "rotate(180deg)" : ""}}>▼ 展开</span></div>
        ...
        {mWeapons.length > 0 && <div className="flex gap-3 mt-3 flex-wrap">{mWeapons.map(w => <div key={w.metadata?.name as string} className="bg-[#f5f5f5] rounded px-3 py-2 text-xs"><span className="font-medium">{w.metadata?.name}</span><span className="text-[#999] ml-2">{w.stats.kills?.displayValue} 击杀 · {w.stats.killsPerMinute?.displayValue} KPM · {w.stats.accuracy?.displayValue} ACC</span></div>)}</div>}
      </div>
        {isExpanded && <div className="border-t border-[#e8e8e8] p-5 bg-[#fafafa] space-y-5">
          {ov && <div><h4 className="text-xs font-semibold text-[#666] mb-2">概览</h4><div className="grid grid-cols-5 gap-4">{["kills","kd","killsPerMinute","scorePerMinute","score"].map(k => ov.stats[k as keyof typeof ov.stats] ? <div key={k} className="text-center"><div className="text-lg font-bold">{ov.stats[k as keyof typeof ov.stats]!.displayValue}</div><div className="text-[10px] text-[#999]">{k==="kills"?"击杀":k==="kd"?"K/D":k==="killsPerMinute"?"KPM":k==="scorePerMinute"?"SPM":"得分"}</div></div> : null)}</div></div>}
